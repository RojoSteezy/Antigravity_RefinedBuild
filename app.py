...
def api_check_key():
    has_any = bool(ANTHROPIC_API_KEY or GOOGLE_API_KEY or OPENAI_API_KEY)
    return jsonify(has_key=has_any)

@app.route('/api/chat', methods=['POST'])
def api_chat():
    if not (ANTHROPIC_API_KEY or GOOGLE_API_KEY or OPENAI_API_KEY):
        ...
        if 'tradui' in last.lower() or 'arabe' in last.lower():
            reply = "🔑 **Mode démo** — Pour activer l'IA, créez un fichier `.env` à la racine et ajoutez votre clé :\n\n`ANTHROPIC_API_KEY=...` ou `GOOGLE_API_KEY=...` ou `OPENAI_API_KEY=...`"
        else:
            reply = "🔑 **Mode démo actif** — Je suis votre assistant ANPDP spécialisé en loi 18-07.\n\nConfigurez une clé API (Anthropic, Gemini ou OpenAI) pour m'activer complètement."
    ...
    try:
        ...
        if not reply:
            return jsonify(error="Aucune clé API configurée ou échec de l'appel"), 400
